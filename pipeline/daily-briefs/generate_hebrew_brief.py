"""
Wrapper for generate_brief_video.py with Hebrew (RTL) support.
Generates daily brief videos with Hebrew text, RTL layout, and he-IL TTS voice.

Usage:
    python generate_hebrew_brief.py <brief.json> [--language he]
"""
import asyncio
import json
import os
...
TOPIC_COLORS = {
    "dotnet-he": (138, 43, 226),
    "ai-he": (0, 200, 255),
    "cloud-he": (0, 180, 120),
    "dev-he": (255, 165, 0),
    "security-he": (220, 20, 60),
    "gamedev-he": (50, 205, 50),
    "dotnet": (138, 43, 226),
    "ai": (0, 200, 255),
    "cloud": (0, 180, 120),
    "dev": (255, 165, 0),
...
def _get_font(size):
    """Return a TrueType font supporting Hebrew, falling back to default."""
    candidates = [
...
def _rtl_text(draw, position, text, font, fill, anchor_right=True):
    """Draw text with RTL alignment (anchored to right side)."""
    if anchor_right and _is_hebrew(text):
        bbox = draw.textbbox((0, 0), text, font=font)
        tw = bbox[2] - bbox[0]
        x = WIDTH - 120 - tw  # Mirror position from right margin
        draw.text((x, position[1]), text, fill=fill, font=font)
    else:
        draw.text(position, text, fill=fill, font=font)


def _rtl_multiline(draw, position, text, font, fill, max_width=40):
    """Draw multi-line text with RTL alignment."""
    lines = textwrap.fill(text, width=max_width).split('\n')
    y = position[1]
    line_height = font.size + 8 if hasattr(font, 'size') else 40
    for line in lines:
        _rtl_text(draw, (position[0], y), line, font, fill, anchor_right=True)
        y += line_height


def _draw_topic_badge(draw, topic_id, topic_name, y=40, rtl=False):
    """Draw a colored badge. For RTL, place in top-left corner."""
    ...
    if rtl:
        x = 40
        draw.rounded_rectangle([x, y, x + tw + pad * 2, y + 50], radius=12, fill=color)
...
def make_title_slide(brief, output_path, is_hebrew=False):
    """Create the title/intro slide with RTL support."""
    img = _gradient_bg()
    draw = ImageDraw.Draw(img)
    topic_id = brief["topic_id"]
    topic_name = brief["topic_name"]

    _draw_topic_badge(draw, topic_id, topic_name, rtl=is_hebrew)
    ...
    if is_hebrew:
        _rtl_text(draw, (120, 300), topic_name, title_font, "white")
        _rtl_text(draw, (120, 400), brief["date"], date_font, (180, 180, 220))
        _rtl_text(draw, (120, 480), f"{n} סיפורים היום", count_font, (140, 140, 180))
        _rtl_text(draw, (120, HEIGHT - 80), "TechAI Explained", brand_font, (100, 100, 140), anchor_right=False)
    else:
...
def make_item_slide(brief, index, item, output_path, is_hebrew=False):
    """Create a slide for a single news item with RTL support."""
    img = _gradient_bg()
    draw = ImageDraw.Draw(img)
    topic_id = brief["topic_id"]
    topic_name = brief["topic_name"]
    ...
    if is_hebrew:
        _rtl_text(draw, (120, 40), f"סיפור {index + 1} מתוך {total}", num_font, (140, 140, 180))
        _rtl_multiline(draw, (120, 160), item["headline"], headline_font, "white", max_width=40)
        _rtl_multiline(draw, (120, 480), item["summary"], summary_font, (200, 200, 230), max_width=65)
    else:
        draw.text((120, 40), f"Story {index + 1} of {total}", fill=(140, 140, 180), font=num_font)
...
def make_outro_slide(brief, output_path, is_hebrew=False):
    """Create the outro/CTA slide with RTL support."""
    img = _gradient_bg()
    draw = ImageDraw.Draw(img)
    ...
    if is_hebrew:
        _rtl_text(draw, (120, 340), "תודה שצפיתם!", title_font, "white")
        _rtl_text(draw, (120, 440), "הירשמו לעדכונים יומיים", sub_font, (180, 180, 220))
    else:
        draw.text((120, 340), "Thanks for watching!", fill="white", font=title_font)
...
async def generate_tts(text, output_path, voice=None):
    """Generate TTS audio using edge-tts with the specified voice."""
    if voice is None:
        voice = TTS_VOICE_HE if _is_hebrew(text) else TTS_VOICE_EN
...
def build_tts_script(brief):
    ...
    for i, item in enumerate(brief["items"], 1):
        parts.append(f"סיפור {i}. {item['headline']}. {item['summary']}"
                     if _is_hebrew(brief.get("intro", ""))
                     else f"Story {i}. {item['headline']}. {item['summary']}")
...
def compose_video_ffmpeg(slides, audio_files, output_path, per_slide_secs=None):
    ...
    if concat_result.returncode != 0:
        print(f"ffmpeg concat error: {concat_result.stderr[:300]}", file=sys.stderr)
...
async def generate_video(brief_json_path, language="en"):
    ...
    voice = TTS_VOICE_HE if is_hebrew else TTS_VOICE_EN
    ...
    make_title_slide(brief, title_slide, is_hebrew=is_hebrew)
    ...
    for i, item in enumerate(brief["items"]):
        ...
        make_item_slide(brief, i, item, slide_path, is_hebrew=is_hebrew)
    ...
    make_outro_slide(brief, outro_slide, is_hebrew=is_hebrew)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate daily brief video (Hebrew/English)")
    parser.add_argument("brief_json", help="Path to the brief JSON file")
    parser.add_argument("--language", default="en", choices=["en", "he"],
                        help="Language: 'en' for English (default), 'he' for Hebrew RTL")
